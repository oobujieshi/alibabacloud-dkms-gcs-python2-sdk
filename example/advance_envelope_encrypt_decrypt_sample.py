...
def decrypt(data_key_iv, encrypted_data_key):
...
def local_encrypt(key, plaintext):
...
def local_decrypt(key, iv, ciphertext):
...
def save_envelope_cipher_persist_object(out_cipher_text):
    # 用户自行保存输出的密文对象
    pass
